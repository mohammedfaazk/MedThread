...
import { Brain, Search, AlertCircle, CheckCircle, TrendingUp, FileText } from 'lucide-react';
...
export default function AIDiseaseDetectivePage() {
  ...
  const analyzeSymptoms = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch('/api/v1/ai-detective/analyze', {
        method: 'POST',
        ...
        {!showResults ? (
          ...
          <div>
            ...
            <div className="space-y-6">
              {results.map((result, index) => (
                <div key={index} className="bg-white rounded-lg shadow p-6">
                  ...
                  {result.relatedSymptoms.length > 0 && (
                  ...
                  <div className="mt-4 pt-4 border-t bg-red-50 -mx-6 -mb-6 px-6 py-4 rounded-b-lg">
                    <h4 className="font-semibold text-red-900 mb-1 flex items-center gap-2">
                      <AlertCircle className="h-4 w-4" />
                      When to Seek Care
                    </h4>
                    <p className="text-sm text-red-800">{result.whenToSeekCare}</p>
                  </div>
                </div>
              ))}
            </div>
            ...
            <div className="mt-6 bg-white rounded-lg shadow p-6 text-center">
              <h3 className="font-semibold mb-2">Need Professional Advice?</h3>
              <p className="text-gray-600 mb-4">
                Connect with verified doctors on MedThread for personalized medical guidance
              </p>
              <button
                onClick={() => router.push('/doctor-feed')}
