  ...
  const fetchData = async () => {
    try {
      ...
      const [peakRes, bottleneckRes] = await Promise.all([
        fetch('/api/platform-analytics/peak-usage?days=30', { headers }),
        fetch('/api/platform-analytics/bottlenecks', { headers })
      ]);

      const peakData = await peakRes.json();
      const bottleneckData = await bottleneckRes.json();

      if (peakData.success) setPeakUsage(peakData.data);
      if (bottleneckData.success) setBottlenecks(bottleneckData.data);
    } catch (error) {
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              ...
              <div className="space-y-2">
                {peakUsage && Object.entries(peakUsage.peakHours)
                  .sort(([, a], [, b]) => b - a)
                  .slice(0, 5)
                  .map(([hour, count]) => (
            ...
            <div>
              ...
              <div className="space-y-2">
                {peakUsage && Object.entries(peakUsage.peakDays)
                  .sort(([, a], [, b]) => b - a)
                  .map(([day, count]) => (
          ...
          <div className="mt-6 p-4 bg-blue-50 rounded-lg">
            <p className="text-sm">
              ...
              {peakUsage?.averageActiveUsers.toFixed(0) || 0} per day
            </p>
          </div>
        </CardContent>
      </Card>
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-3xl font-bold">
              {peakUsage?.averageActiveUsers.toFixed(0) || 0}
            </p>
        ...
        <Card>
          ...
          <CardContent>
            <p className="text-3xl font-bold text-orange-600">
              {(bottlenecks?.highBouncePosts.length || 0) + (bottlenecks?.slowDoctors.length || 0)}
            </p>
