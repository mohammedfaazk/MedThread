...
export default function PublicHealthDashboardRealtime() {
  ...
  useEffect(() => {
    // Subscribe to real-time health analytics
    subscribe('health');
  ...
  useEffect(() => {
    if (analyticsData.health.trending.length > 0) {
      setTrending(analyticsData.health.trending);
      setLoading(false);
    }
    if (analyticsData.health.alerts.length > 0) {
      setAlerts(analyticsData.health.alerts);
    }
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            <span className="ml-auto text-xs font-normal text-gray-500">Live</span>
          </CardTitle>
          <CardDescription>
            Most reported symptoms in real-time
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
            {trending.map((item, index) => (
              <div key={item.symptom} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg transition-all hover:bg-gray-100">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Automated prevention recommendations based on real-time data
          </CardDescription>
        </CardHeader>
        <CardContent>
          <div className="space-y-3">
            {trending.slice(0, 3).map((item) => (
              <div key={item.symptom} className="p-4 bg-blue-50 border-l-4 border-blue-500 rounded">
                ...
                <p className="text-sm text-gray-700">
                  {item.count} users reporting {item.symptom} symptoms. 
