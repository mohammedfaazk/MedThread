...
import { Search, Leaf, ChevronDown, CheckCircle2 } from 'lucide-react'

export function NavbarJWT() {
  ...
  const displayRole = isDoctorPending 
    ? 'DOCTOR (PENDING)' 
    : isDoctorVerified
    ? 'VERIFIED DOCTOR'
    : role?.replace('_', ' ')
              ...
              >
                ...
                <div className="flex flex-col items-start hidden lg:flex">
                  <div className="flex items-center gap-1 mb-0.5">
                    ...
                    {isDoctorVerified && <CheckCircle2 className="w-3 h-3 text-blue-500 fill-blue-500/10" />}
                  </div>
                  <span className="text-[10px] text-slate-500 font-medium uppercase tracking-wider">{displayRole}</span>
                </div>
