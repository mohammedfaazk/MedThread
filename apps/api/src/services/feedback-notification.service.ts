...
export class FeedbackNotificationService {
   ...
   */
  async checkFeedbackNeeded(patientId: string, conversationId?: string, appointmentId?: string) {
    const twoDaysAgo = new Date();
    twoDaysAgo.setDate(twoDaysAgo.getDate() - 2);
   ...
   */
  async getDoctorFeedbackStats(doctorId: string) {
   ...
   */
  async getPendingFeedbacks(limit: number = 50) {
    const feedbacks = await prisma.patientFeedback.findMany({
      where: {
        status: 'NOT_YET'
      },
