...
export class AvailabilityService {
   ...
   * Find next opening day
   */
  private findNextOpenDay(hours: ClinicHours[], currentDay: number): string | null {
  ...
  calculateNextAvailableSlot(
    ...
    for (let daysAhead = 0; daysAhead < maxDaysAhead; daysAhead++) {
      ...
      const isBooked = bookedSlots.some(booked => 
        Math.abs(booked.getTime() - slotTime.getTime()) < 60000 // Within 1 minute
      );
   ...
   */
  isEmergencyAvailable(availability: any): boolean {
   ...
   */
  acceptsInsurance(availability: any, insuranceProvider: string): boolean {
    ...
    return availability.insurance_accepted.some(
      (provider: string) => provider.toLowerCase() === insuranceProvider.toLowerCase()
    );
  }
}
