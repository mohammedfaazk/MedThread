...
export class EnhancedAnalyticsService {
  ...
  async getDoctorSpecialtyDistribution() {
    ...
    const distribution = doctors.reduce((acc: any, doctor) => {
    ...
    const percentages = Object.entries(distribution).map(([specialty, count]) => ({
      ...
      percentage: ((count as number / total) * 100).toFixed(2)
    }));
   ...
   */
  async analyzeCommunityActivity(communityId?: string) {
   ...
   */
  async getDoctorPublicStats(doctorId: string) {
   ...
   */
  private calculatePortfolioScore(performance: any): number {
    const {
      curedPatientCount = 0,
      notYetCount = 0,
   ...
   */
  async getDoctorPortfolio(doctorId: string) {
    ...
    const conversionsByComment = conversions.reduce((acc: any, conv) => {
   ...
   */
  async trackClinicVisit(doctorId: string, patientId: string) {
    await prisma.doctorPerformance.upsert({
      where: { doctorId },
      create: {
        doctorId,
  ...
  async recalculateAllPortfolioScores() {
    ...
    for (const performance of performances) {
      ...
      updates.push(
        prisma.doctorPerformance.update({
  ...
  async getTopDoctors(options: {
    ...
    const performanceMap = performances.reduce((acc: any, p) => {
      acc[p.doctorId] = p;
      return acc;
    }, {});
