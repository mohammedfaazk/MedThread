...
interface CmeCreditsConfig {
  QUALITY_ANSWER: number;
  PEER_REVIEWED_ANSWER: number;
  CASE_DISCUSSION: number;
  EDUCATIONAL_THREAD: number;
  EXPERT_PANEL: number;
  RESEARCH_CONTRIBUTION: number;
  MENTORSHIP: number;
}

export class CmeCreditsService {
  // Credit values for each activity type
  private readonly CREDIT_VALUES: CmeCreditsConfig = {
    QUALITY_ANSWER: 0.5,
   ...
   */
  async awardCredits(activity: CmeActivity) {
    ...
    if (!doctor || doctor.role !== 'DOCTOR') {
      throw new ValidationError('Only doctors can earn CME credits');
    }

    if (doctor.doctorVerificationStatus !== 'APPROVED') {
      throw new ValidationError('Doctor must be verified to earn CME credits');
    }
    ...
    const existingData = (user?.verificationDocuments as any) || {};
   ...
   */
  async checkAndAwardForReply(replyId: string) {
   ...
   */
  private async checkReplyQuality(reply: any): Promise<{
    eligible: boolean;
    activityType: CmeActivityType;
    score: number;
  }> {
    let score = 0;
    let activityType: CmeActivityType = 'QUALITY_ANSWER';

    // Check upvotes from verified doctors
    const doctorUpvotes = await prisma.vote.count({
      where: {
        commentId: reply.id,
        value: 1,
        user: {
   ...
   */
  async getDoctorCmeCredits(doctorId: string) {
    const user = await prisma.user.findUnique({
      where: { id: doctorId },
      select: {
    ...
    const cmeData = (user.verificationDocuments as any) || {};
    const cmeRecords = cmeData.cmeRecords || [];
    const currentYear = new Date().getFullYear();

    // Calculate credits by year
    const creditsByYear = cmeRecords.reduce((acc: any, record: any) => {
    ...
    const creditsByType = cmeRecords.reduce((acc: any, record: any) => {
      acc[record.activityType] = (acc[record.activityType] || 0) + record.creditsEarned;
      return acc;
    }, {});

    // Get pending verifications
    const pendingCredits = cmeRecords.filter((r: any) => r.verificationStatus === 'PENDING');
   ...
   * Get CME leaderboard
   */
  async getCmeLeaderboard(timeframe: 'month' | 'year' | 'all' = 'month', limit = 10) {
    ...
    const leaderboard = users.map((user: any) => {
      ...
      if (timeframe === 'all') {
        ...
        credits = cmeRecords
          .filter((r: any) => new Date(r.earnedAt).getFullYear() === currentYear)
          .reduce((sum: number, r: any) => sum + r.creditsEarned, 0);
      } else {
        credits = cmeRecords
          .filter((r: any) => {
   ...
   */
  async generateCertificate(doctorId: string, activityId: string) {
    const user = await prisma.user.findUnique({
      where: { id: doctorId },
      select: {
        username: true,
    ...
    const cmeData = (user.verificationDocuments as any) || {};
    const cmeRecords = cmeData.cmeRecords || [];
    const activity = cmeRecords.find((r: any) => r.id === activityId);

    if (!activity) {
      throw new NotFoundError('CME activity not found');
    }
   ...
   */
  async getCmeOpportunities(doctorId: string) {
    const doctor = await prisma.user.findUnique({
      where: { id: doctorId },
    ...
    const opportunities = await prisma.medicalThread.findMany({
      where: {
        status: 'OPEN',
        tags: {
