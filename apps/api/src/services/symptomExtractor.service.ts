...
export function extractSymptoms(text: string): string[] {
  ...
  return Object.entries(SYMPTOM_DICTIONARY)
    .filter(([, keywords]) => keywords.some(kw => lower.includes(kw)))
    .map(([symptom]) => symptom);
}

export function extractSeverity(text: string): number {
  const lower = text.toLowerCase();
  if (['severe','critical','extreme','unbearable','emergency'].some(w => lower.includes(w))) return 5;
  if (['very bad','really bad','high','intense','strong'].some(w => lower.includes(w))) return 4;
  if (['moderate','medium','somewhat','quite'].some(w => lower.includes(w))) return 3;
  if (['mild','slight','little','minor'].some(w => lower.includes(w))) return 2;
  return 3;
}