...
export class DoctorProfileAnalyticsService {
  ...
  async getPatientAcquisitionGraph(doctorId: string) {
    ...
    while (currentDate <= endDate) {
      ...
      const newPatients = Array.from(uniquePatients.values()).filter(date => 
        date >= currentDate && date <= monthEnd
      ).length;
   ...
   */
  async getAverageReplyTime(doctorId: string) {
   ...
   */
  async getDailyActivityGraph(doctorId: string) {
    ...
    return {
      ...
      lastActiveText,
      peakHour: hourlyActivity.reduce((max, current) => 
   ...
   */
  async getComprehensiveDoctorStats(doctorId: string) {
...
export const doctorProfileAnalyticsService = new DoctorProfileAnalyticsService();