...
interface TrendingInsight {
  type: 'symptom' | 'condition' | 'medication' | 'regional';
  title: string;
  description: string;
  growthRate: number;
  caseCount: number;
...
export class HealthInsightsService {
   ...
   */
  async generateTrendingSymptoms(timeframe: 'week' | 'month' = 'week'): Promise<TrendingInsight[]> {
   ...
   */
  async generateRegionalAlerts(): Promise<any[]> {
   ...
   */
  async analyzeMedicationPatterns(): Promise<MedicationPattern[]> {
    const threads = await prisma.medicalThread.findMany({
      where: {
    ...
    threads.forEach(thread => {
      thread.replies.forEach(reply => {
        ...
        medications.forEach(med => {
          ...
          if (sentiment === 'positive') data.positive++;
          else if (sentiment === 'negative') data.negative++;
          else data.neutral++;
  ...
  async getDiagnosticPatterns() {
    ...
    grouped.forEach((cases, symptoms) => {
      if (cases.length >= 3) {
        const misdiagnoses = new Map<string, number>();
        cases.forEach(c => {
   ...
   */
  async getDoctorInsightsDashboard(doctorId: string, specialty?: string) {
    const [
      trendingSymptoms,
      regionalAlerts,
      medicationPatterns,
    ...
    const specialtyFilter = specialty || (await prisma.user.findUnique({
      where: { id: doctorId },
  ...
  private getStartDate(timeframe: 'week' | 'month'): Date {
    const now = new Date();
    if (timeframe === 'week') {
  ...
  private getPreviousStartDate(timeframe: 'week' | 'month'): Date {
    const now = new Date();
    if (timeframe === 'week') {
  ...
  private countSymptoms(threads: any[]): Record<string, number> {
  ...
  private extractSymptoms(symptomsData: any): string[] {
    ...
    if (symptomsData && typeof symptomsData === 'object') {
      return Object.values(symptomsData).flat() as string[];
    }
    return [];
  }
  ...
  private analyzeSentiment(text: string, medication: string): 'positive' | 'negative' | 'neutral' {
    ...
    if (medIndex === -1) return 'neutral';

    const context = lowerText.substring(
      Math.max(0, medIndex - contextWindow),
      Math.min(lowerText.length, medIndex + medLower.length + contextWindow)
    );
  ...
  private extractSideEffects(text: string, medication: string): string[] {
  ...
  private calculateSeverity(count: number, growthRate: number): 'low' | 'medium' | 'high' | 'critical' {
    if (count > 50 && growthRate > 100) return 'critical';
    if (count > 30 || growthRate > 75) return 'high';
    if (count > 15 || growthRate > 40) return 'medium';
    return 'low';
  }
}
