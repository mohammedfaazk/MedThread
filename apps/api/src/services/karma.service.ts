...
const KARMA_MILESTONES: KarmaMilestone[] = [
...
export class KarmaService {
   ...
   */
  async updateUserKarma(userId: string): Promise<KarmaBreakdown> {
   ...
   */
  async getUserKarma(userId: string): Promise<KarmaBreakdown & { milestone: KarmaMilestone }> {
   ...
   */
  getKarmaMilestone(karma: number): KarmaMilestone {
   ...
   */
  getAllMilestones(): KarmaMilestone[] {
   ...
   */
  async getSpecialtyLeaderboard(specialty: string, limit = 20, offset = 0) {
    const [doctors, total] = await Promise.all([
      prisma.user.findMany({
        where: {
          specialty: {
   ...
   */
  async getUserRank(userId: string): Promise<{ rank: number; totalUsers: number; percentile: number }> {
    const user = await prisma.user.findUnique({
      where: { id: userId },
