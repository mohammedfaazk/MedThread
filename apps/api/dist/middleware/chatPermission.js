...
exports.checkMessageRateLimit = exports.canAccessConversation = exports.validateChatAccess = void 0;
...
const validateChatAccess = async (req, res, next) => {
    try {
        ...
        console.log('[ChatPermission] Doctor verification check:', {
            ...
            required: database_1.DoctorVerificationStatus.APPROVED
        });
        if (appointment.doctor.doctorVerificationStatus !== database_1.DoctorVerificationStatus.APPROVED) {
            return res.status(403).json({
        ...
        console.log('[ChatPermission] Appointment status check:', {
            ...
            required: database_1.AppointmentStatus.APPROVED
        });
        if (appointment.status !== database_1.AppointmentStatus.APPROVED) {
            return res.status(403).json({
...
exports.validateChatAccess = validateChatAccess;
/**
 ...
 */
const checkMessageRateLimit = async (userId, conversationId) => {
    try {
        const windowStart = new Date();
        windowStart.setMinutes(windowStart.getMinutes() - 1); // 1 minute window
        // Count messages in the last minute
        const messageCount = await database_1.prisma.message.count({
            where: {
                senderId: userId,
                conversationId,
                createdAt: {
