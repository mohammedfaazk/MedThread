    ...
    async getPatientStats(req, res) {
        try {
            ...
            const [user, totalAppointments, upcomingAppointments, completedAppointments, totalPosts, totalComments] = await Promise.all([
                ...
                database_1.prisma.appointment.count({ where: { patientId: userId } }),
                database_1.prisma.appointment.count({ where: { patientId: userId, scheduledAt: { gte: now }, status: { in: ['PENDING', 'CONFIRMED'] } } }),
                database_1.prisma.appointment.count({ where: { patientId: userId, status: 'COMPLETED' } }),
