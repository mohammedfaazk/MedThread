...
exports.replyRouter.post('/', async (req, res) => {
    try {
        const data = createReplySchema.parse(req.body);
        const author = await database_1.prisma.user.findUnique({
        ...
        const reply = await database_1.prisma.threadReply.create({
            data: {
...
// Upvote a reply
exports.replyRouter.post('/:id/upvote', async (req, res) => {
    try {
        const { id } = req.params;
        const { userId } = req.body;
        // Check if user already voted
        const existingVote = await database_1.prisma.vote.findFirst({
            where: {
                commentId: id,
                userId
            }
        ...
        if (existingVote) {
            if (existingVote.value === 1) {
                // Remove upvote
                await database_1.prisma.vote.delete({ where: { id: existingVote.id } });
        ...
        const reply = await database_1.prisma.threadReply.update({
            where: { id },
...
// Downvote a reply
exports.replyRouter.post('/:id/downvote', async (req, res) => {
    try {
        const { id } = req.params;
        const { userId } = req.body;
        // Check if user already voted
...
exports.replyRouter.post('/:id/helpful', async (req, res) => {
    try {
        const { id } = req.params;
        const { userId } = req.body;
        ...
        const updated = await database_1.prisma.threadReply.update({
            where: { id },
...
exports.replyRouter.post('/:id/best-answer', async (req, res) => {
    try {
        const { id } = req.params;
        const { userId } = req.body;
        const reply = await database_1.prisma.threadReply.findUnique({
        ...
        await database_1.prisma.threadReply.updateMany({
            where: {
                threadId: reply.threadId,
                id: { not: id }
            },
...
// Get reply votes
exports.replyRouter.get('/:id/votes', async (req, res) => {
    try {
        const { id } = req.params;
        ...
        let userVote = null;
        if (userId) {
            const vote = await database_1.prisma.vote.findFirst({
                where: {
                    commentId: id,
                    userId: userId
                }
...
exports.default = exports.replyRouter;
