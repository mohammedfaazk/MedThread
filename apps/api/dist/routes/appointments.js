...
router.get('/doctors/:doctorId/availability', async (req, res) => {
    try {
        ...
        try {
            ...
            availability = await database_1.prisma.availability.findMany({
            ...
            const bookedAppointments = await database_1.prisma.appointment.findMany({
                where: {
                    doctorId,
...
router.post('/availability', async (req, res) => {
    try {
        ...
        let availability;
        try {
            availability = await database_1.prisma.availability.create({
                data: {
                    doctorId,
                    dayOfWeek,
...
router.post('/book', async (req, res) => {
    try {
        ...
        let appointment;
        try {
            appointment = await database_1.prisma.appointment.create({
                data: {
                    patientId,
                    doctorId,
                    startTime: new Date(startTime),
            ...
            if (appointment && appointment.doctorId === doctorId) {
                ...
                if (status === 'APPROVED') {
                    ...
                    try {
                        ...
                        await database_1.prisma.doctorPerformance.upsert({
                    ...
                    try {
                        ...
                        const existingConversation = await database_1.prisma.conversation.findUnique({
                            where: { appointmentId: id }
                        });
                        if (!existingConversation) {
                            await database_1.prisma.conversation.create({
                                data: {
                                    appointmentId: id,
                                    participants: {
                                        connect: [
        ...
        if (index !== -1) {
            ...
            if (status === 'APPROVED') {
                ...
                try {
                    // Directly increment conversion count for appointment approvals
                    await database_1.prisma.doctorPerformance.upsert({
                        where: { doctorId: mockStore_1.appointmentsStore[index].doctorId },
                        create: {
                            doctorId: mockStore_1.appointmentsStore[index].doctorId,
                            conversionCount: 1,
                            clinicVisitCount: 1,
                            totalPostsCommented: 0,
                            totalCommentsCount: 0
    ...
    try {
        ...
        try {
            const where = role === 'doctor'
                ? { doctorId: userId }
                : { patientId: userId };
            dbAppointments = await database_1.prisma.appointment.findMany({
                where,
                include: {
                    patient: { select: { id: true, username: true, avatar: true } },
                    doctor: { select: { id: true, username: true, avatar: true, specialty: true } }
