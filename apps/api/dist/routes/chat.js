...
router.get('/conversations', async (req, res) => {
    try {
        ...
        let dbConversations = [];
        try {
            dbConversations = await database_1.prisma.conversation.findMany({
                where: {
                    participants: {
                        some: { id: userId }
                    }
...
router.get('/conversations/:id/messages', async (req, res) => {
    try {
        ...
        let dbMessages = [];
        try {
            dbMessages = await database_1.prisma.message.findMany({
                where: {
                    conversationId: id,
...
router.post('/messages', async (req, res) => {
    try {
        ...
        if (!conversation) {
            conversation = mockStore_1.conversationsStore.find((c) => c.id === conversationId);
        ...
        try {
            ...
            mockStore_1.messagesStore.push({
                id: message.id,
