...
exports.commentService = {
     ...
     */
    sortByDoctorProximity(comments, patientPincode) {
        const proximityTier = (doctorPincode) => {
            if (!patientPincode || !doctorPincode)
                return 4;
            if (doctorPincode === patientPincode)
                return 0;
            if (doctorPincode.slice(0, 3) === patientPincode.slice(0, 3))
                return 1;
            if (doctorPincode.slice(0, 2) === patientPincode.slice(0, 2))
                return 2;
            if (doctorPincode[0] === patientPincode[0])
                return 3;
            return 4;
        };
    ...
    async getCommentsByAuthor(authorId, limit = 20, offset = 0) {
        const comments = await database_1.prisma.comment.findMany({
            where: {
                authorId,
                isRemoved: false,
