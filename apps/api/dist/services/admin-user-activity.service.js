...
exports.adminUserActivityService = exports.AdminUserActivityService = void 0;
...
class AdminUserActivityService {
     ...
     */
    async getUserActivityTimeGraphs(userId, timeframe = 'hourly') {
     ...
     */
    generateHourlyActivityGraph(user, activities) {
        ...
        const peakHour = hourlyData.reduce((max, current) => current.totalActivity > max.totalActivity ? current : max);
     ...
     */
    generateWeeklyActivityGraph(user, activities) {
        ...
        const peakDay = weeklyData.reduce((max, current) => current.totalActivity > max.totalActivity ? current : max);
        const totalActivities = activities.length;
        const activityByType = {
            posts: activities.filter(a => a.type === 'post').length,
            comments: activities.filter(a => a.type === 'comment').length,
     ...
     */
    async compareUserActivities(userIds, timeframe = 'hourly') {
        const comparisons = [];
     ...
     */
    aggregateUserActivities(activities, timeframe) {
...
exports.AdminUserActivityService = AdminUserActivityService;
exports.adminUserActivityService = new AdminUserActivityService();
