...
class NotificationService {
    ...
    async createNotification(params) {
        ...
        try {
            ...
            const sanitizedMetadata = (0, sanitize_1.sanitizeNotificationMetadata)(metadata);
            // Sanitize URL if present
            if (sanitizedMetadata.link) {
                sanitizedMetadata.link = (0, sanitize_1.sanitizeUrl)(sanitizedMetadata.link);
            }
            ...
            const notifications = await database_1.prisma.$transaction(unblockedRecipients.map((recipientId) => database_1.prisma.notifications.create({
                data: {
                    type,
                    recipientId,
                    actorId,
            ...
            try {
                const { emailQueueService } = await Promise.resolve().then(() => __importStar(require('./email-queue.service')));
    ...
    async markAsRead(notificationId, userId) {
        try {
            ...
            const notification = await database_1.prisma.notifications.findFirst({
                where: {
                    id: notificationId,
                    recipientId: userId,
                },
            ...
            await database_1.prisma.notifications.update({
    ...
    async markAllAsRead(userId) {
        try {
            const result = await database_1.prisma.notifications.updateMany({
                where: {
                    recipientId: userId,
                    isRead: false,
                    isDeleted: false,
    ...
    async getUnreadCount(userId) {
        try {
            ...
            // Fetch from database
            const count = await database_1.prisma.notifications.count({
                where: {
                    recipientId: userId,
                    isRead: false,
                    isDeleted: false,
