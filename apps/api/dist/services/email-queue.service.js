...
class EmailQueueService {
    ...
    async enqueueInstantEmail(notificationId, userId) {
        try {
            ...
            await database_1.prisma.email_queue.create({
                data: {
                    userId,
                    notificationId,
                    type: 'instant',
    ...
    async enqueueInstantEmailBatch(notifications) {
        try {
            ...
            if (jobs.length > 0) {
                await database_1.prisma.email_queue.createMany({
                    data: jobs
                });
    ...
    async processQueue() {
        ...
        try {
            ...
            const jobs = await database_1.prisma.email_queue.findMany({
                where: {
                    status: 'pending',
                    attempts: {
    ...
    async processJob(job) {
        ...
        try {
            ...
            await database_1.prisma.email_queue.update({
                where: { id: job.id },
                data: {
                    attempts,
            ...
            let success = false;
            if (job.type === 'instant') {
                success = await this.emailService.sendNotificationEmail(job.User, job.notifications);
            }
            if (success) {
                // Mark as sent
                await database_1.prisma.email_queue.update({
    ...
    async getQueueStats() {
        const [pending, sent, failed, total] = await Promise.all([
            database_1.prisma.email_queue.count({ where: { status: 'pending' } }),
            database_1.prisma.email_queue.count({ where: { status: 'sent' } }),
            database_1.prisma.email_queue.count({ where: { status: 'failed' } }),
            database_1.prisma.email_queue.count()
        ]);
    ...
    async retryFailedJobs() {
        const result = await database_1.prisma.email_queue.updateMany({
            where: {
                status: 'failed'
            },
            data: {
