...
class RevenueService {
    ...
    async getSubscriptionTiers() {
        const tiers = await database_1.prisma.$queryRaw `
      SELECT *
    ...
    async purchaseSubscription(data) {
        ...
        // Get tier details
        const tier = await database_1.prisma.$queryRaw `
    ...
    `;
      ...
      `;
        ...
        const subscriptionId = await database_1.prisma.$queryRaw `
    ...
    `;
        ...
        const commissionId = await database_1.prisma.$queryRaw `
    ...
    async getDoctorSubscription(doctorId) {
        const subscription = await database_1.prisma.$queryRaw `
      SELECT 
        ds.*,
        st.tier_name,
        st.display_name,
    ...
    async createAdvertisement(adData) {
        const { advertiserName, advertiserEmail, adType, adTitle, adDescription, imageUrl, clickUrl, targetSpecialties, targetLocations, placementPages, pricingModel, costPerImpression, costPerClick, totalBudget, dailyBudget, startDate, endDate } = adData;
        await database_1.prisma.$executeRaw `
    ...
    `;
        ...
        const ads = await database_1.prisma.$queryRawUnsafe(query);
        return ads;
    }
    /**
    ...
    async recordAdImpression(data) {
        ...
        // Get ad pricing
        const ad = await database_1.prisma.$queryRaw `
     ...
     * Record ad click
     */
    async recordAdClick(adId, impressionId) {
        // Get ad pricing
        const ad = await database_1.prisma.$queryRaw `
     ...
     * Create data insight
     */
    async createDataInsight(insightData) {
        const { insightType, insightTitle, insightDescription, dataSummary, aggregatedData, dataSource, timePeriodStart, timePeriodEnd, sampleSize, regionType, regionName, specialty, accessLevel, price } = insightData;
        await database_1.prisma.$executeRaw `
     ...
     */
    async getPlatformRevenue(periodType, startDate, endDate) {
        let query = `
      SELECT *
    ...
    `;
        ...
        const revenue = await database_1.prisma.$queryRawUnsafe(query, ...params);
        return revenue;
    }
    /**
     * Get doctor revenue summary
     */
    async getDoctorRevenueSummary(doctorId, startDate, endDate) {
        let query = `
      SELECT 
    ...
    `;
        ...
        const summary = await database_1.prisma.$queryRawUnsafe(query, ...params);
        return summary.length > 0 ? summary[0] : null;
    }
    /**
     * Aggregate daily revenue (called by cron)
     */
    async aggregateDailyRevenue(date) {
        await database_1.prisma.$executeRaw `
