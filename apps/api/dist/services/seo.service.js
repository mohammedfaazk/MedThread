...
class SEOService {
     ...
     */
    generateDoctorSlug(username, specialty, city) {
        const parts = [
            'dr',
     ...
     */
    generateDoctorSchema(doctor, clinic, rating) {
        return {
     ...
     */
    generateReviewSchema(review, doctor) {
        return {
            '@context': 'https://schema.org',
            '@type': 'Review',
            itemReviewed: {
     ...
     */
    async createDoctorSEOProfile(doctorId) {
        // Get doctor info
        const doctor = await database_1.prisma.$queryRaw `
    ...
    `;
        ...
        // Get rating info
        const rating = await database_1.prisma.$queryRaw `
     ...
     */
    async generateTopDoctorsBlogPost(city, specialty) {
     ...
     */
    generateBlogContent(doctors, city, specialty) {
        ...
        doctors.forEach((doc, index) => {
            ...
            if (doc.yearsOfExperience)
                content += `**Experience:** ${doc.yearsOfExperience} years\n`;
            if (doc.bio)
                content += `\n${doc.bio}\n`;
     ...
     * Track SEO analytics
     */
    async trackSEOAnalytics(entityType, entityId, data) {
