...
exports.doctorRankingService = exports.DoctorRankingService = void 0;
const database_1 = require("@medthread/database");
class DoctorRankingService {
    /**
     * Calculate overall rank score for a doctor
     */
    calculateRankScore(rating, weights = {
        overallRating: 0.30,
        responseTime: 0.15,
        successRate: 0.20,
        satisfaction: 0.20,
        ...
        const rankScore = (overallScore * weights.overallRating) +
    ...
    `;
        ...
        const doctors = await database_1.prisma.$queryRawUnsafe(query, ...params);
        return doctors;
    }
    /**
     ...
     */
    async getMostHelpfulInSpecialty(specialty, limit = 10) {
        const doctors = await database_1.prisma.$queryRaw `
      SELECT 
        u.id,
        u.username,
    ...
    async updateDoctorRating(doctorId) {
        ...
        const reviewStats = await database_1.prisma.$queryRaw `
      SELECT 
    ...
    `;
        ...
        const responseTime = await database_1.prisma.$queryRaw `
      SELECT 
    ...
    `;
        ...
        const helpfulStats = await database_1.prisma.$queryRaw `
      SELECT 
    ...
    `;
        ...
        const consultationStats = await database_1.prisma.$queryRaw `
      SELECT 
    ...
    `;
        ...
        doctors.forEach(doctor => {
            ...
            if (doctor.city) {
                ...
                if (!regionGroups.has(cityKey))
                    regionGroups.set(cityKey, []);
            ...
            if (doctor.state) {
                ...
                if (!regionGroups.has(stateKey))
                    regionGroups.set(stateKey, []);
            ...
            if (doctor.country) {
                ...
                if (!regionGroups.has(countryKey))
                    regionGroups.set(countryKey, []);
                regionGroups.get(countryKey).push({ ...doctor, region_type: 'country', region_name: doctor.country });
            }
        });
        // Clear existing rankings
        await database_1.prisma.$executeRaw `DELETE FROM "DoctorRegionalRank"`;
        // Insert new rankings
    ...
    `;
        ...
        await database_1.prisma.$executeRaw `DELETE FROM "DoctorTrending" WHERE week_start_date = ${weekStart}`;
        // Calculate trending scores and insert
        for (const doctor of doctors) {
            const trendingScore = (doctor.reply_count_7d * 10) +
    ...
    `;
        ...
        await database_1.prisma.$executeRaw `DELETE FROM "DoctorRisingStar"`;
        for (const doctor of doctors) {
            ...
            const risingStarScore = (doctor.overall_rating * 20) +
                (ratingVelocity * 30) +
                (replyVelocity * 10) +
...
exports.DoctorRankingService = DoctorRankingService;
exports.doctorRankingService = new DoctorRankingService();
