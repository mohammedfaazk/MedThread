...
exports.regionalSymptomAnalyticsService = exports.RegionalSymptomAnalyticsService = void 0;
const database_1 = require("@medthread/database");
class RegionalSymptomAnalyticsService {
    /**
     ...
     */
    async resolvePincodeToLocation(pincode) {
        const pincodeMap = {
            // Tamil Nadu — Chennai
     ...
     */
    extractSymptomsFromPost(title, content) {
        ...
        const detectedSymptoms = symptomKeywords.filter(symptom => combinedText.includes(symptom));
    ...
    calculateSeverity(symptoms) {
        ...
        const hasHighSeverity = symptoms.some(s => highSeveritySymptoms.some(hs => s.includes(hs)));
        const hasMediumSeverity = symptoms.some(s => mediumSeveritySymptoms.some(ms => s.includes(ms)));
        if (hasHighSeverity)
            return 'HIGH';
    ...
    async getRegionalSymptomHeatmap(options) {
        ...
        reports.forEach(report => {
            ...
            if (report.pincode)
                group.pincodes.add(report.pincode);
            // Count symptoms
        ...
        return {
            ...
            summary: {
                ...
                totalSymptoms: new Set(reports.flatMap(r => r.symptoms)).size
            }
        };
    }
    /**
     ...
     */
    calculateAlertLevel(count, severity) {
        const highSeverityRatio = severity.HIGH / count;
        if (count >= 50 && highSeverityRatio >= 0.3)
            return 'CRITICAL';
        if (count >= 20 && highSeverityRatio >= 0.2)
            return 'HIGH';
        if (count >= 10)
            return 'MEDIUM';
        return 'LOW';
    ...
    async getTrendingSymptoms(days = 7) {
        ...
        reports.forEach(report => {
            report.symptoms.forEach(symptom => {
                ...
                stats.count++;
                if (report.city)
                    stats.regions.add(report.city);
                if (report.state)
                    stats.states.add(report.state);
                stats.severity[report.severity]++;
            });
        });
     ...
     */
    async getLocationSymptomDetails(location, locationLevel) {
        const thirtyDaysAgo = new Date();
        thirtyDaysAgo.setDate(thirtyDaysAgo.getDate() - 30);
        const where = {
    ...
    async generateRegionalHealthAlerts() {
        ...
        reports.forEach(report => {
            ...
            if (report.severity === 'HIGH')
                alert.highSeverityCount++;
            if (report.city)
                alert.affectedCities.add(report.city);
            report.symptoms.forEach(symptom => {
     ...
     */
    async getSymptomsByPincode(pincode, scope, timeWindow = 'month') {
        ...
        const where = { reportedAt: { gte: since } };
        if (scope === 'area')
        ...
        reports.forEach(report => {
            ...
            report.symptoms.forEach(symptom => {
                if (!symptomCounts[symptom])
                    symptomCounts[symptom] = { count: 0, high: 0, medium: 0, low: 0 };
                symptomCounts[symptom].count++;
                if (report.severity === 'HIGH')
                    symptomCounts[symptom].high++;
                else if (report.severity === 'MEDIUM')
                    symptomCounts[symptom].medium++;
                else
                    symptomCounts[symptom].low++;
            });
        });
     ...
     */
    async collectFromPatientPost(postId, userId, symptoms, duration) {
        ...
        const existing = await database_1.prisma.symptomReport.findFirst({ where: { postId } });
        if (existing) {
            return database_1.prisma.symptomReport.update({
        ...
        return database_1.prisma.symptomReport.create({
            data: {
                userId,
                postId,
                symptoms: normalizedSymptoms,
...
exports.RegionalSymptomAnalyticsService = RegionalSymptomAnalyticsService;
exports.regionalSymptomAnalyticsService = new RegionalSymptomAnalyticsService();
