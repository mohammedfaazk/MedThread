...
function parseAge(group) {
    if (!group)
        return null;
    if (group.includes('60+'))
        return 65;
 ...
 */
function computeCalories(profile, hintCalories) {
    const missing = [];
    ...
    if (!sex || sex === 'other')
        missing.push('biological sex');
    if (!act || !ACTIVITY_MULT[act])
 ...
 */
function computeMacros(calories, conditions) {
...
function computeRiskLevel(conditions) {
    const high = ((conditions.some(c => /diabetes/i.test(c)) && conditions.some(c => /blood pressure|hypertension/i.test(c))) ||
 ...
 */
function reconcileMealCalories(meals, targetCalories) {
    ...
    return meals.map((meal, i) => {
        ...
        const dishSum = meal.dishes.reduce((s, d) => s + (d.calories ?? 0), 0);
        const scale = dishSum > 0 ? mealTarget / dishSum : 1;
        let dishRunning = 0;
        const dishes = meal.dishes.map((d, j) => {
 ...
 */
function reconcileMacros(meals, formulaMacros) {
...
// Prompt builder
// ─────────────────────────────────────────────────────────────────────────────
function buildPrompt(p, calc, conditions, allergies, riskLevel) {
...
class DietPlanService {
    async generateDietPlan(userId, hintCalories) {
        try {
            const healthProfile = await database_1.prisma.healthProfile.findUnique({ where: { userId } });
            if (!healthProfile) {
            ...
            if (!aiResponse.success)
                return aiResponse;
            const dietPlan = await database_1.prisma.dietPlan.create({
                data: {
                    userId,
            ...
            await database_1.prisma.dietPlan.updateMany({
    ...
    async getActiveDietPlan(userId) {
        try {
            const dietPlan = await database_1.prisma.dietPlan.findFirst({
    ...
    async saveDietPlan(userId, dietPlanId) {
        try {
            const dietPlan = await database_1.prisma.dietPlan.update({
     ...
     */
    async callGroqWithRetry(healthProfile, calc) {
        if (!groq) {
            ...
            try {
                const completion = await groq.chat.completions.create({
                    messages: [
                        {
                            role: 'system',
                            content: 'You are a clinical nutrition engine. You output ONLY a single valid JSON object. ' +
