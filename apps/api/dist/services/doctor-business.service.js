...
exports.doctorBusinessService = exports.DoctorBusinessService = void 0;
const database_1 = require("@medthread/database");
class DoctorBusinessService {
    /**
     * Get comprehensive business analytics for a doctor
     */
    async getDoctorAnalytics(doctorId, filter = {}) {
     ...
     */
    async calculateRatingTrend(doctorId, startDate, endDate) {
        const ratings = await database_1.prisma.$queryRaw `
      SELECT 
        date,
        average_rating
      FROM "DoctorBusinessAnalytics"
     ...
     */
    async getRevenueBreakdown(doctorId, startDate, endDate) {
        const revenue = await database_1.prisma.$queryRaw `
      SELECT 
        transaction_type,
     ...
     */
    async getPatientRetention(doctorId) {
        const retention = await database_1.prisma.$queryRaw `
      SELECT 
        retention_status,
     ...
     */
    async createPromotion(doctorId, data) {
     ...
     */
    async activatePromotion(promotionId, paymentId) {
     ...
     */
    async getActivePromotions(doctorId) {
        const promotions = await database_1.prisma.$queryRaw `
      SELECT 
        dp.*,
        CASE 
    ...
    `;
        ...
        if (promo.promotion_type === 'top_search') {
            const topSearch = await database_1.prisma.$queryRaw `
      ...
      `;
            ...
            const sponsored = await database_1.prisma.$queryRaw `
        SELECT 
     ...
     */
    async trackPromotionImpression(promotionId, type) {
        await database_1.prisma.$executeRaw `
      UPDATE "DoctorPromotion"
     ...
     */
    async trackPromotionClick(promotionId, type) {
        await database_1.prisma.$executeRaw `
      UPDATE "DoctorPromotion"
      SET clicks = clicks + 1,
          ctr = CASE 
    ...
    async getFeaturedDoctors(limit = 10) {
        const featured = await database_1.prisma.$queryRaw `
      SELECT 
        u.id,
        u.username,
        u.avatar,
     ...
     */
    async updateDailyAnalytics(doctorId, date = new Date()) {
        const yesterday = new Date(date);
        yesterday.setDate(yesterday.getDate() - 1);
        // Get profile views from SEO
        const seoViews = await database_1.prisma.$queryRaw `
    ...
    `;
        ...
        const consultations = await database_1.prisma.$queryRaw `
      SELECT 
    ...
    `;
        // Get patient metrics
        const patients = await database_1.prisma.$queryRaw `
      SELECT 
...
exports.DoctorBusinessService = DoctorBusinessService;
exports.doctorBusinessService = new DoctorBusinessService();
