...
exports.patientJourneyService = exports.PatientJourneyService = void 0;
...
class PatientJourneyService {
    ...
    `;
      ...
      `;
        ...
        // Create new journey
        const journey = await database_1.prisma.$queryRaw `
     ...
     */
    async trackBookingInitiation(patientId, doctorId, appointmentId) {
        await database_1.prisma.$executeRaw `
      UPDATE "PatientJourney"
     ...
     */
    async completeQuestionnaire(appointmentId, answers) {
        await database_1.prisma.$executeRaw `
    ...
    async sendPendingReminders() {
        const reminders = await database_1.prisma.$queryRaw `
     ...
     */
    async requestReview(appointmentId) {
        const appointment = await database_1.prisma.appointment.findUnique({
            where: { id: appointmentId }
        });
        if (!appointment)
        ...
        const request = await database_1.prisma.$executeRaw `
    ...
    async sendPendingReviewRequests() {
        const requests = await database_1.prisma.$queryRaw `
        ...
        for (const request of requests) {
            try {
                await email_service_1.emailService.sendEmail({
                    ...
                    subject: `How was your consultation with Dr. ${request.doctor_name}?`,
                    html: `
            <p>Hi ${request.patient_name},</p>
            <p>We hope your consultation went well! We'd love to hear about your experience.</p>
            <p><a href="${process.env.FRONTEND_URL}/review/${request.appointment_id}">Leave a Review</a></p>
          `
                });
                await database_1.prisma.$executeRaw `
    ...
    async scheduleFollowUp(data) {
        const followUp = await database_1.prisma.$queryRaw `
     ...
     */
    async trackCTAImpression(ctaId) {
        await database_1.prisma.$executeRaw `
      UPDATE "BookingCTA"
     ...
     */
    async getDoctorCTAs(doctorId) {
        const ctas = await database_1.prisma.$queryRaw `
...
exports.PatientJourneyService = PatientJourneyService;
exports.patientJourneyService = new PatientJourneyService();
