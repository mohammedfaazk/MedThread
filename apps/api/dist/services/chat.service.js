...
class ChatService {
    ...
    async createMessage(input) {
        ...
        // Validate content - either content or attachment must be provided
        if ((!content || content.trim().length === 0) && !attachment) {
            throw new Error('Message content or attachment is required');
        }
        // Only validate content length if content is provided
        ...
        try {
            ...
            await notificationService.createNotification({
                ...
                metadata: {
                    preview: content ? content.substring(0, 100) : (attachment ? '[Attachment]' : '[Message]'),
    ...
    async editMessage(input) {
        ...
        // Message exists, proceed with edit
        ...
        try {
            const io = (0, socket_1.getSocketInstance)();
            // Add isEdited flag for frontend display
     ...
     * Note: Since Conversation model doesn't have isActive field, we just emit the event
     */
    async deactivateConversation(conversationId, reason) {
        ...
        return { id: conversationId, reason };
    }
}
